...
def initialize_all(args):
    ...
    if not os.path.exists(args.source_directory_path):
        ...
        raise Exception('The path of the source directory does not exist.')
    ...
    if not os.path.exists(args.destination_directory_path):
        ...
        os.makedirs(name=args.destination_directory_path)
    ...
    if args.config not in configs:
        logger.error(f'There is no config called {args.config}.')
        raise Exception(f'There is no config called {args.config}.')
