...
class ServerThread(threading.Thread):
    ...
    def run(self, *args, **kwargs):
        ...
        while is_shutdown == False:
            ...
            client_thread = ClientThread(client_socket, client_index,
                                         self.parameters)
...
class ClientThread(threading.Thread):
    ...
    def __init__(self, client_socket, client_index, parameters, *args,
                 **kwargs):
    ...
    def run(self, *args, **kwargs):
        ...
        if self.client_index == 0:
            ...
            if model_name == 'LJPBert':
                ...
                article_sources_table = self.parameters[
                ...
                while is_shutdown == False:
                    ...
                    if client_message == 'shutdown':
                        ...
                        result = model(data=fact,
                                       mode='serve',
                                       acc_result=None)

                        # The size of accusation_result is [number_of_class].
                        article_result = torch.max(input=result['article'],
                                                   dim=2)[1]
                        article_source_result = torch.max(
                            input=result['article_source'], dim=2)[1]
                        accusation_result = torch.max(
                            input=result['accusation'], dim=2)[1]
