...
from abstractive_text_summarization.formatter.bart import BartFormatter
...
def initialize_formatter(config, task=None, *args, **kwargs):
    ...
    try:
        ...
        if config.get('model', 'model_name') == 'LJSBart':
    ...
    formatters = {'BartFormatter': BartFormatter}

    if formatter_name in formatters:
        formatter = formatters[formatter_name](config)
    else:
